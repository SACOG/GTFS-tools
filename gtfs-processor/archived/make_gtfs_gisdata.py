...
Purpose: create shapefiles of all unique trip shapes from GTFS data, with basic
        operating information for each trip shape

        FUTURE IMPROVEMENTS
        -Give ability to create lines by stitching together stop points, in case
        operator doesn't provide shapes.txt file.
        -Ability to get count of departures, VSH within specific time periods
...
class MakeGTFSGISData(object):
    ...
    def get_prd_opdata(self, str_prd_start, str_prd_end, groupby_attrs=[]):
        ...
        ***NEXT STEP - NEED TO FILTER OUT TO ONLY INCLUDE WEEKDAY SERVICE TYPE OR
        ...
        f_trip_ttmins = 'tt_mins' # travel time in minutes for trip
        ...
        out_cols = gbcols + [f_tripstart, f_trip_ttmins]
        
        df_groupby = df_merged[out_cols].groupby(gbcols)
        
        #get count of trips and sum of travel time in period
        df_out = df_groupby.agg({f_tripstart: "count", f_trip_ttmins: "sum"}) \
            .reset_index()
        ...
        df_out[f_headway] = prd_dur_mins / df_out[f_tripcnt_prd]  # calculate headway in mins
...
if __name__ == '__main__':
    gtfs_folder = r'I:\Projects\Darren\CMPs\CMP2020\Transit Data\Yolobus\gtfs'
    ...
    if output_type not in ('csv', 'gis'):
        ...
        df = gtfso.get_prd_opdata(start_time, end_time, groupby_attrs=['route_id', 'route_short_name', 'shape_id'])
